...
Patient::Patient(std::string name, std::string secName, std::string otch, int cardNumb, int diseaseNumb)
{
	...
	std::cout << "Patient " << GetFio() << " with card: " << GetCardNumb() << " with disease " << GetDiseaseNumb() << " created" << std::endl;
}
...
void Observer::GetInfo(Patient& patient)
{
	std::cout << "Patient with card number " << patient._cardNumb << std::endl;
}
